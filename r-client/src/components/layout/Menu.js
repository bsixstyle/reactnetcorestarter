...
function Menu(props) {
    const _parent = UserServices.getMenu().filter((menu) => menu.parentId === null);
      const renderParent = (menu, i) => {
        const _child = UserServices.getMenu().filter((child) => child.parentId === menu.id);
        const renderChild = (child, ic) => {
          return <Link onClick={props.hideMenu} key={ic} to={child.action}>{child.name}</Link>
        }
        ...
        return <Link onClick={props.hideMenu} key={i} to={menu.action}>{menu.name}</Link>
      }     
